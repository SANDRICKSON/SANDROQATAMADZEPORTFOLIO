...
def verify_email(token):
    ...
    # ვერიფიკაციის შემდეგ ვამატებთ ელფოსტას session-ში
    session['verified_email'] = email

    flash('Email successfully verified! You can now send messages.', 'success')
    return redirect(url_for('contact'))

@app.route('/contact', methods=['GET', 'POST'])
def contact():
    form = ContactForm()
    email = form.email.data if form.email.data else session.get('verified_email')
    ...
    if form.validate_on_submit():
        ...
        # ვერიფიკაციის შემდეგ session-იდან ელფოსტის წაშლა
        session.pop('verified_email', None)

        return redirect(url_for('contact'))

    return render_template('contact.html', form=form, verified_email=session.get('verified_email'))
